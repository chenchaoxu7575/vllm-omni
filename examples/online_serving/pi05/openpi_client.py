...
"""Minimal OpenPI client for the π0.5 VLA policy server.

Connects to a running π0.5 server (see README.md), sends a single robot
...
The wire format is identical to π0's. The π0.5-specific part happens entirely
server-side: the state is quantile-normalized, discretized into 256 bins and
serialized into the prompt, so the model receives no state tensor. The client
still sends a plain ``state`` float array.
...
try:
    ...
    raise ImportError("π0.5 OpenPI example requires `openpi-client`.") from exc
...
# π0.5 / pi05_base camera identities (must match the server's image_feature_keys).
CAMERA_KEYS = (
...
def _make_dummy_obs(prompt: str, session_id: str, image_size: int = 224) -> dict[str, Any]:
    """Build a single π0.5 observation with blank cameras + zero state.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="π0.5 OpenPI client")
    ...
    logger.info("Done — π0.5 OpenPI round-trip OK.")
    return 0
